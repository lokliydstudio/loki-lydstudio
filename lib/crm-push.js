...
const { sendNativePush } = require("./crm-native-push");
...
async function sendWebPush(actorEmail, input, options = {}) {
...
async function sendPush(actorEmail, input, options = {}) {
  const notification = notificationPayload(input);
  const [web, native] = await Promise.all([
    sendWebPush(actorEmail, notification, options),
    sendNativePush(actorEmail, notification, options),
  ]);
  return { sent: web.sent + native.sent, removed: web.removed + native.removed };
}
