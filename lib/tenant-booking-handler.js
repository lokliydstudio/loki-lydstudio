...
const { sendMail } = require("./crm-mail");
const { sendPush } = require("./crm-push");
const { isConfigured, mutateCollection, readCollection, writeCollection } = require("./crm-store");
const {
  ACCOUNT_STATUSES,
  bookingConflict,
  challengeCookie,
...
async function tenantForRequest(req, accounts) {
  ...
  return account?.status === "approved"
    && account.studio === session.studio
    && (account.sessionVersion || 0) === (session.sessionVersion || 0)
    ? account : null;
}
...
async function verifyCodeHandler(req, res) {
  ...
  const accounts = await readCollection(USER_COLLECTION);
  const index = accounts.findIndex((item) => item.id === challenge.accountId
    && item.email === email
    && item.studio === challenge.studio
    && (item.sessionVersion || 0) === (challenge.sessionVersion || 0)
...
async function sessionHandler(req, res) {
  ...
  res.setHeader("Set-Cookie", sessionCookie(account));
...
async function calendarHandler(req, res) {
  ...
  const mutation = await mutateCollection(BOOKING_COLLECTION, (bookings) => {
    if (bookings.filter((item) => item.createdById === account.id && item.date >= todayInOslo()).length >= MAX_FUTURE_BOOKINGS_PER_USER) {
      return { error: { status: 429, message: "Du har nådd grensen for fremtidige bookinger. Kontakt Loki Lydstudio." } };
    }
    const conflict = bookingConflict(bookings, booking);
    if (conflict) return { error: { status: 409, message: `Tiden overlapper med «${conflict.title}» (${conflict.startTime}–${conflict.endTime}).` } };
    return { items: [...bookings, booking] };
  });
  if (mutation.error) return res.status(mutation.error.status).json({ error: mutation.error.message });
...
async function adminBookingHandler(req, res) {
  ...
  const mutation = await mutateCollection(BOOKING_COLLECTION, (bookings) => {
    ...
    if (index < 0) return { error: { status: 404, message: "Bookingen ble ikke funnet." } };
    return { items: bookings.filter((_, itemIndex) => itemIndex !== index), result: bookings[index] };
  });
  if (mutation.error) return res.status(mutation.error.status).json({ error: mutation.error.message });
...
module.exports = async function handler(req, res) {
  ...
  const action = String(req.query?.action || "").replace(/^tenant-/, "");
  try {
...
module.exports._test = { tenantForRequest, todayInOslo };
