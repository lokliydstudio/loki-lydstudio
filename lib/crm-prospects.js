...
async function discoverProspectContact(prospect, options = {}) {
  ...
  if (!response.ok) {
    const failure = await response.json().catch(() => ({}));
    const detail = String(failure?.error?.message || failure?.error || "").trim().slice(0, 300);
    throw new Error(`Kontaktsøket feilet hos AI Gateway (${response.status})${detail ? `: ${detail}` : "."}`);
  }
  const data = await response.json();
  const parsed = JSON.parse(responseText(data));
  const contactSourceUrl = cleanUrl(parsed.contactSourceUrl);
  if (!contactSourceUrl) return { configured: true, contact: {} };
  return {
    configured: true,
    contact: {
      contactName: cleanText(parsed.contactName, 160),
      contactRole: cleanText(parsed.contactRole, 160),
      publicEmail: cleanEmail(parsed.publicEmail),
      publicPhone: cleanPhone(parsed.publicPhone),
      contactSourceUrl,
